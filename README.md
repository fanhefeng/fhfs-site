...
fhf 的个人网站：收录文章、自研软件与动效实验。
暖纸色、编辑部式排版、一盏琥珀色的灯；深色主题是「闭馆后」的同一间画廊。
完整设计语言（材质、动效语法、每页的叙事）见 `docs/DESIGN.md`。

## 版面

- **首页** —— 硬着陆时先是那块霓虹大门（每 session 一次，推门从圆环里穿过去），然后是
  一整屏纸：宣言、一句自我介绍、会眨眼的小人、站点唯一的主按钮，和一行「现居 / 找到我 /
  关于我」；接着是正刊三节——软件架子、近期文章、此刻（最新一条说说、最新一则实验、
  最近一次发布）。苔藓树根的那扇窗 2026-09-24 起退回实验室（/lab/approach），首页不再
  加载 three.js。没走大门的话，开场点灯仪式每会话一次。
...
- **/lab** —— 三十三则动效研究，**一则只放一种效果**（同一个插件、同一层叠着出现、同一个
  组件的两条分支，都各自一则），索引分三架（`LabEntry.group`，每架一个标题一句话）。
  **为实验室做的**（01–08）：滚动帧序列、溶解、融化文字、苔藓树根、色散按钮、可拖拽的 3D
  工作台、镜头畸变滑块、Seb's 式的霓虹招牌。**站点在用的**（09–26）：横穿屏幕
  的一句话、报头的行遮罩、y:24 揭示、标题的解码、中文标题的行遮罩、磁吸、描边、模切白边、
  贴纸墙、撕开的贴纸、散开又聚回的字、噪点里的名字、首页的推近、分段控件的药丸、筛选重排、
  铜像、版本履历、放映厅。**壳层**（27–33）：开灯 / 阅读进度 / 扇形菜单 /
  开灯仪式 / 推门 / 换页的雾 / 灵动岛。序号一路排下去，架只是歇眼的地方。每则按路由单独
...
- **/moments** —— 「峰言峰语」（栏目名取自「疯言疯语」，两个疯都换成名字里的峰；英文名
  Peak Talk）：QQ 空间式的说说板，几行字加一个时间，按年排、按文集筛。条目本身照旧叫说说。
  头 242 条是从一言 App 搬来的两本文集（峰言峰语 / 默认文集），进这一页放 Lovely Day
  （Jurrivh）。
...
- **/films** —— 电影：看了很多遍的几部，一部一页。事实条、一段简介、记得的台词、一面六列的
  ...
  （上下两部、片尾曲《一生所爱》）、《不能说的秘密》（原声带里的《路小雨》）与《爱乐之城》
  （Mia & Sebastian's Theme）；进每一页换那部片的唱片。
  旧地址 `/odyssey` 308 到 `/films/odyssey`。
- **/life** —— 「生活」：房间的目录。峰言峰语 / 偶像 / 电影 / 秘密（以及以后每一间）一行一间，
...
`/admin/copy` 因此是照着语言文件生成的：885 行都能改，按页面分组，
**输入框清空 = 恢复默认**，不是把那处变成空白。想真的留空，就得把默认改成空的。
...
- /films/secret 的十二张剧照：TMDB 上《不能说的秘密》（id 20342）只有六张 backdrop，其中四张是同一
  ...
  （42 张，3:2 的片场照片）：八张官方剧照 + 两张 TMDB backdrop（单车、父亲的吉他）+ 两帧电影
...
- /films/lala 的十二张剧照放在 `public/films/lala/`，清单与尺寸在
  ...
  致敬之用，页面上有署名。音乐是门口那首 Mia & Sebastian's Theme（见 /lab/neon 一条）。
