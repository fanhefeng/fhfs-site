...
>   好奇。」（`grove.lede`，两份语言文件）；09-22 的小人从正刊末节提到大字旁边——lg 以上占右侧
>   一列 224px，以下站在大字上方（`flex-col-reverse`，DOM 里仍在文字之后，Tab 序不先落到他身上）；
>   小人由页面渲染后作 `avatar` 传入，`ChibiArt` 的路径因此仍是服务端组件、不进客户端包。主按钮
>   不动；折线下那行 mono 从「现居」扩成「现居 · 找到我（GitHub / RSS / Email）· 关于我 →」，
>   `AboutTeaser` 删除，它的三样东西都到了这里。
> - **正刊**：01 自研软件（主按钮指向它，先兑现）→ 02 最近文章 → 03 **此刻**（`home/NowStrip.tsx`，
