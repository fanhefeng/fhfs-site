...
export function LiquidPill({ children, height, className, label, href, pad = 1.744, mixRef, base, onPress }: Props) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      setShared(pScene.u);
      gl.uniform1f(pScene.u.uHover ?? null, Math.min(1, m.base + (1 - m.base) * hover));
      ...
      gl.uniform1f(pComp.u.uGlowGain ?? null, m.glow);
      ...
      gl.uniform1f(pComp.u.uPunch ?? null, m.punch);
      gl.uniform1f(pComp.u.uRimGain ?? null, m.rim);
      drawTo(null);
    };
